...
async function testeLoginSucesso() {
  ...
  try {
    ...
    await driver.wait(
      until.elementLocated(By.css('[data-testid="header-login-link-desktop"]')),
      10000
    );

    await driver
      .findElement(By.css('[data-testid="header-login-link-desktop"]'))
      .click();
    ...
    let emailInput = await driver.findElement(
      By.css('[data-testid="login-email-input"]')
    );
    let passwordInput = await driver.findElement(
      By.css('[data-testid="login-password-input"]')
    );
    let submitButton = await driver.findElement(
      By.css('[data-testid="login-submit-button"]')
    );
...
async function testeLoginFalha() {
  ...
  try {
    ...
    await driver.wait(
      until.elementLocated(By.css('[data-testid="login-api-message"]')),
      10000
    );
    let errorMessage = await driver
      .findElement(By.css('[data-testid="login-api-message"]'))
      .getText();

    if (errorMessage.includes('Email ou senha inválidos')) {
      console.log('-----------------------------------------');
      console.log(
        'TESTE DE LOGIN COM FALHA... PASSOU! (Mensagem de erro exibida)'
      );
      console.log('-----------------------------------------');
    } else {
